# Mergeability gate (conflict-resolution phase) - orchestrator-side reference
...
The gate exists because a PR marked READY-TO-MERGE at end
...
- 5b (fan-out, one subagent per CONFLICTING PR): dispatch the
  `../assets/conflict-resolution-prompt.md` spawn body. WAIT for all returns.
...
`../assets/conflict-resolution-prompt.md` spawn body. Subagents run in
...
- Does NOT re-run the autopilot-pr-review-worker. The autopilot-pr-merge-worker
