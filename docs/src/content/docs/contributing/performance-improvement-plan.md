...
| ---: | --- | --- | --- |
| 8 | **Use blobless clones for full-package cache misses.** **Status:** Implemented; controlled transfer and latency comparison remains pending. New full and sparse misses share one blobless bare, legacy full bares remain reusable, and checkout hydration uses process-scoped authenticated promisor configuration. **Evidence:** cache-miss Git object transfer is measured; host-specific savings remain a hypothesis. | High potential for large repositories. Hosts that explicitly reject partial clone fall back to a full bare. Returned checkouts retain only the local bare URL, so later Git commands cannot lazy-fetch with ambient credentials. | Compare transferred bytes, cache size, and cold install/update medians. Tests require identical package contents, shared bare reuse, no persistent upstream promisor URL, and successful full-clone fallback. |
